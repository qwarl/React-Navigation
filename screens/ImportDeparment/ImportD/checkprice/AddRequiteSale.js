...
const AddRequiteSale = ({ route, navigation }) => {
  ...
  const [importInfo, setImportInfo] = useState(route.params.item);
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const res1 = await clientCheckPriceImport.delete(url + id);
      const res = await clientImport.post("/create", {
        ...importInfo,
      });
      if (res.data.success) {
