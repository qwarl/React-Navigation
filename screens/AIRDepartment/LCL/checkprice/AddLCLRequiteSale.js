...
const AddLCLRequiteSale = ({ navigation, route }) => {
  const handleOnChangeText = (value, fieldName) => {
    setLCLInfo({ ...lclInfo, [fieldName]: value });
  };

  const [lclInfo, setLCLInfo] = useState(route.params.item);
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const res1 = await clientCheckPriceAirLCL.delete(url + id);
...
export default AddLCLRequiteSale;
