...
const AddFCLRequiteSale = ({ navigation, route }) => {
  ...
  const [fclInfo, setFclInfo] = useState(route.params.item);
  const submitForm = async () => {
    try {
      const url = "/delete/";
      const id = fclInfo._id;
      const res1 = await clientCheckPriceFCL.delete(url + id);
...
export default AddFCLRequiteSale;
