...
import clientCheckPriceLog from "../../../api/clientCheckPriceLog";

const AddLogRequiteSale = ({ route, navigation }) => {
  ...
  const [logInfo, setLogInfo] = useState(route.params.item);

  console.log(logInfo._id);
  ...
  const submitForm = async () => {
    ...
    try {
      const url = "/delete/";
      const id = logInfo._id;
      const res1 = await clientCheckPriceLog.delete(url + id);
...
export default AddLogRequiteSale;
