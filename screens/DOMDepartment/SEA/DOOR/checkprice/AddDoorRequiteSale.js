...
const AddDoorRequiteSale = ({ navigation, route }) => {
  const [error, setError] = useState("");
  const [seaDoorInfo, setSeaDoorInfo] = useState(route.params.item);
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const res1 = await clientCheckPriceDoor.delete(url + id);
...
export default AddDoorRequiteSale;
